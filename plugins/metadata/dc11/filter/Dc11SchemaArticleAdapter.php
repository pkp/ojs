...
class Dc11SchemaArticleAdapter extends MetadataDataObjectAdapter
{
    ...
    public function &extractMetadataFromDataObject(&$article): MetadataDescription
    {
        ...
        if (!empty($publisherInstitution)) {
            ...
            $publishers = $publication->getData('contextName', null); // Default
        }
        ...
        $includeUrls = ($journal->getData('publishingMode') != Journal::PUBLISHING_MODE_NONE) ||
            $issueAction->subscribedUser($request->getUser(), $journal, null, $article);
        if ($includeUrls) {
            $dc11Description->addStatement(
                'dc:identifier',
        ...
        $sources = $publication->getData('contextName', null);
        ...
        if ($issn = $publication->getData('onlineIssn')) {
            $dc11Description->addStatement(
                'dc:source',
                $issn,
                MetadataDescription::METADATA_DESCRIPTION_UNKNOWN_LOCALE
        ...
        if ($issn = $publication->getData('printIssn')) {
            $dc11Description->addStatement(
                'dc:source',
                $issn,
                MetadataDescription::METADATA_DESCRIPTION_UNKNOWN_LOCALE
