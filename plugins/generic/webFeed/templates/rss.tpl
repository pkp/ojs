...
{foreach name=sections from=$publishedSubmissions item=section key=sectionId}
	{foreach from=$section.articles item=article}
		...
		<item rdf:about="{url page="article" op="view" path=$article->getBestId()}">
			...
			{if ($publication->getData('accessStatus') == $smarty.const.ARTICLE_ACCESS_OPEN || ($publication->getData('accessStatus') == $smarty.const.ARTICLE_ACCESS_ISSUE_DEFAULT && $issue->getAccessStatus() == \APP\issue\Issue::ISSUE_ACCESS_OPEN)) && $article->isCCLicense()}
