...
<body class="pkp_page_{$requestedPage|escape} pkp_op_{$requestedOp|escape}">
	...
	<div id="htmlContainer" class="galley_view{if !$isLatestPublication} galley_view_with_notice{/if}" style="overflow:visible;-webkit-overflow-scrolling:touch">
		...
		{else}
			...
			{/capture}
		{/if}
		<iframe name="htmlFrame" src="{$htmlUrl}" title="{translate key="submission.representationOfTitle" representation=$galley->getLabel()|escape title=$galleyPublication->getLocalizedFullTitle(null)|escape}" allowfullscreen webkitallowfullscreen></iframe>
	</div>
