...
class CounterPlugin extends GenericPlugin
{
    ...
    function register($category, $path)
    {
        ...
        if ($success && $this->getEnabled()) {
            ...
            HookRegistry::register('Templates::User::Index::MyAccount', array($this, 'myCounter'));

            $this->import('CounterReportDAO');
            $this->import('CounterUserDAO');
            $counterReportDao = new CounterReportDAO();
            $counterUserDao = new CounterUserDAO();
            DAORegistry::registerDAO('CounterReportDAO', $counterReportDao);
            DAORegistry::registerDAO('CounterUserDAO', $counterUserDao);
        }
        return $success;
    }
    ...
    /**
     * Show link to my counter stats
     */
    function myCounter($hookName, $args)
    {
        $output =& $args[2];
        $output .= '<li>&#187; <a href="' . Request::url(null, 'counter', 'userStats', null, array('mode' => 'ALL_VIEWINGS')) . '">' . __('plugins.generic.user.mycounter') . '</a></li>';
        return false;
    }
    ...
    function logRequestInline($hookName, $args)
    {
        ...
        // check if IP is related to an institutional subscription
        $user =& Request::getUser();
        $userId = ($user) ? $user->getId() : null;

        if (isset($userId)) {
            /** @var $subscriptionDao IndividualSubscriptionDAO */
        ...
        if (!isset($individualSubscription) || !$individualSubscription->isValid()) {
            /** @var $subscriptionDao InstitutionalSubscriptionDAO */
            $subscriptionDao =& DAORegistry::getDAO('InstitutionalSubscriptionDAO');

            if (Config::getVar('security', 'implicit_auth')) {
                $organisation = Config::getVar('security', 'implicit_auth_header_org');
                if (isset($_SERVER[$organisation])) {
                    $implicitAuthOrg = $_SERVER[$organisation];
                    $subscriptionId = $subscriptionDao->isValidInstitutionalSubscriptionForOrg($implicitAuthOrg, $journalId);
                    if ($subscriptionId) {
                        $institutionalSubscription =& $subscriptionDao->getSubscription($subscriptionId);
                        $userId = $institutionalSubscription->getUserId();
                    }
                }
            } else {
        ...
        if ($userId != null) {
            ...
            $counterReportDao->incrementCount($journal->getJournalId(), (int)strftime('%Y'), (int)strftime('%m'), false, false, $userId, $articleId);
        }
    }
    ...
    function logRequest($hookName, $args)
    {
        ...
        if ($this->isContributingStaff($journal)) return false;
        ...
        if ($userId != null) {
            $this->logCounterHit($session, $article, $galley, $userId);
        } else {
            $this->logCounterHit($session, $article, $galley);
        }
     ...
     * @return bool
     */
    function logCounterHit($session, $article, $galley, $userId = null)
    {
        ...
        /** @var $counterReportDao CounterReportDAO */
        $counterReportDao =& DAORegistry::getDAO('CounterReportDAO');
        $counterReportDao->incrementCount($article->getJournalId(), (int)strftime('%Y'), (int)strftime('%m'), $galley->isPdfGalley(), $galley->isHTMLGalley(), $userId, $article->getArticleId());
        return true;
    }

    /**
     * Checks if this user is a contributing staff member to this journal. If they are then their view of the article will not be added the counter statistic
     * @param $journal object
     * @return bool
     */
    function isContributingStaff(&$journal)
    {
