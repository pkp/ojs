...
class PiwikSettingsForm extends Form {
	...
	function PiwikSettingsForm(&$plugin, $journalId) {
		...
		$this->addCheck(new FormValidatorCustom($this, 'piwikUrl', 'required', 'plugins.generic.piwik.manager.settings.piwikUrlRequired', create_function('$piwikUrl', 'return strpos(trim(strtolower($piwikUrl)), \'http://\') === 0 ? true : false;')));
