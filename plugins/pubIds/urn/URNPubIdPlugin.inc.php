		...
		if (!$urn) {
			...
			switch ($urnSuffixSetting) {
				...
				case 'pattern':
					...
					if ($issue) {
						// %j - journal initials
						$suffixPattern = String::regexp_replace('/%j/', String::strtolower($journal->getLocalizedInitials()), $suffixPattern);
						// %v - volume number
				...
				default:
					if ($issue) {
						$suffixPattern = String::strtolower($journal->getLocalizedInitials());
