...
<div class="block" id="sidebarRTArticleTools">
	...
	{if $journalRt->getAbstract() && $galley && $article->getLocalizedAbstract()}
		<div class="articleToolItem">
			<img src="{$baseUrl}/plugins/blocks/readingTools/icons/abstract.png" class="articleToolIcon" /> <a href="{url router=$smarty.const.ROUTE_PAGE page="article" op="view" path=$articleId}" target="_parent">{translate key="article.abstract"}</a><br />
		</div>
	{/if}
	{if $journalRt->getPrinterFriendly()}
		<div class="articleToolItem">
			<img src="{$baseUrl}/plugins/blocks/readingTools/icons/printArticle.png" class="articleToolIcon" /> <a href="{if !$galley || $galley->isHtmlGalley()}javascript:openRTWindow('{url router=$smarty.const.ROUTE_PAGE page="rt" op="printerFriendly" path=$articleId|to_array:$galleyId}');{else}{url router=$smarty.const.ROUTE_PAGE page="article" op="download" path=$articleId|to_array:$galleyId}{/if}">{translate key="plugins.block.readingTools.printThisArticle"}</a>
		</div>
	{/if}
	{if $journalRt->getViewMetadata()}
		<div class="articleToolItem">
			<img src="{$baseUrl}/plugins/blocks/readingTools/icons/metadata.png" class="articleToolIcon" /> <a href="javascript:openRTWindow('{url router=$smarty.const.ROUTE_PAGE page="rt" op="metadata" path=$articleId|to_array:$galleyId}');">{translate key="rt.viewMetadata"}</a><br />
		</div>
	{/if}
	...
	{if $journalRt->getViewReviewPolicy()}
		<div class="articleToolItem">
			<img src="{$baseUrl}/plugins/blocks/readingTools/icons/editorialPolicies.png" class="articleToolIcon" /> <a href="{url router=$smarty.const.ROUTE_PAGE page="about" op="editorialPolicies" anchor="peerReviewProcess"}" target="_parent">{translate key="rt.reviewPolicy"}</a>
		</div>
	{/if}
	{if $journalRt->getEmailOthers()}
		<div class="articleToolItem">
			<img src="{$baseUrl}/plugins/blocks/readingTools/icons/emailArticle.png" class="articleToolIcon" />
			{if $isUserLoggedIn}<a href="javascript:openRTWindow('{url router=$smarty.const.ROUTE_PAGE page="rt" op="emailColleague" path=$articleId|to_array:$galleyId}');">{translate key="plugins.block.readingTools.emailThisArticle"}</a>
	...
	{if $journalRt->getEmailAuthor()}
		<div class="articleToolItem">
			<img src="{$baseUrl}/plugins/blocks/readingTools/icons/emailArticle.png" class="articleToolIcon" />
			{if $isUserLoggedIn}<a href="javascript:openRTWindow('{url router=$smarty.const.ROUTE_PAGE page="rt" op="emailAuthor" path=$articleId|to_array:$galleyId}');">{translate key="rt.emailAuthor"}</a>
	...
	{if $enableComments}
		<div class="articleToolItem">
			...
			{else}<a href="{url router=$smarty.const.ROUTE_PAGE page="comment" op="add" path=$article->getId()|to_array:$galleyId}">{translate key="plugins.block.readingTools.postComment"}</a>{/if}
		</div>
	{/if}
</div>
{/if}
