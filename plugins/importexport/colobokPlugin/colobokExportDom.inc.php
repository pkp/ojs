...
class colobokExportDom {
	...
	function &generateArticleDom(&$doc, &$journal, &$issue, &$section, &$article) {
		...
		if (is_array($article->getSubject(null))) foreach ($article->getSubject(null) as $locale => $keyword) {
			...
			if ($locale=='ru_RU'){
				$kwdGroupsrus =& XMLCustomWriter::createElement($doc, 'keywords');
				XMLCustomWriter::appendChild($root, $kwdGroupsrus);
				XMLCustomWriter::setAttribute($kwdGroupsrus, 'language', 'rus');
			...
			if ($locale=='en_US'){
				$kwdGroupseng =& XMLCustomWriter::createElement($doc, 'keywords');
				XMLCustomWriter::setAttribute($kwdGroupseng, 'language', 'eng');
				XMLCustomWriter::appendChild($root, $kwdGroupseng);
				foreach ($subjects as $keyword) {
