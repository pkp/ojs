    ...
    public function &process(&$pubObjects)
    {
        ...
        foreach ($pubObjects as $pubObject) { /** @var Submission|Publication $pubObject */
            ...
            if ($context->getData(Context::SETTING_DOI_VERSIONING)) {
                $recordNode->appendChild($node = $doc->createElement('fullTextUrl', htmlspecialchars($request->getDispatcher()->url($request, Application::ROUTE_PAGE, null, 'article', 'view', [$article->getId(), $publication->getData('versionStage'), $publication->getData('versionMajor')], urlLocaleForPage: ''), ENT_COMPAT, 'UTF-8')));
            } else {
