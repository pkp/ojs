{**
 * @file plugins/importexport/medra2cr/templates/issues.tpl
 *
	...
	<form action="{plugin_url path="process"}" method="post" id="issuesForm">
		...
		<table width="100%" class="listing">
			...
			{iterate from=issues item=issue}
				{if $issue->getData('medra2cr::registeredDoi')}
				...
				<tr valign="top">
					...
					<td align="right"><nobr>
						{if $hasCredentials}
							...
							{if $issue->getData('medra2cr::registeredDoi')}<a href="{plugin_url path="process" issueId=$issue->getId() params=$testMode target="issue" reset=true}" title="{translate key="plugins.importexport.medra.resetDescription"}" class="action">{translate key="plugins.importexport.medra.reset"}</a>{/if}
						{/if}
