{**
 * @file plugins/importexport/medra2cr/templates/articles.tpl
 *
...
<!-- articles.tpl -->
<div id="articles">
	<p>{translate key="plugins.importexport.medra2cr.workOrProduct"}</p>
	<form action="{plugin_url path="process"}" method="post" id="articlesForm">
		...
		<table width="100%" class="listing">
			...
			{iterate from=articles item=articleData}
				...
				{if $article->getData('medra2cr::registeredDoi')}
				...
				<tr valign="top">
					...
					<td align="right"><nobr><!--  update or register -->
						{if $hasCredentials}
							...
							{if $article->getData('medra2cr::registeredDoi')}<a href="{plugin_url path="process" articleId=$article->getId() params=$testMode target="article" reset=true}" title="{translate key="plugins.importexport.medra2cr.resetDescription"}" class="action">{translate key="plugins.importexport.medra2cr.reset"}</a>{/if}
						{/if}
