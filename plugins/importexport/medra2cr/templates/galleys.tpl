{**
 * @file plugins/importexport/medra2cr/templates/galleys.tpl
 *
	...
	<form action="{plugin_url path="process"}" method="post" id="galleysForm">
		...
		<table width="100%" class="listing">
			...
			{iterate from=galleys item=galleyData}
				...
				{if $galley->getData('medra2cr::registeredDoi')}
				...
				<tr valign="top">
					...
					<td align="right"><nobr>
						{if $hasCredentials}
							...
							{if $galley->getData('medra2cr::registeredDoi')}<a href="{plugin_url path="process" galleyId=$galley->getId() params=$testMode target="galley" reset=true}" title="{translate key="plugins.importexport.medra2cr.resetDescription"}" class="action">{translate key="plugins.importexport.medra2cr.reset"}</a>{/if}
						{/if}
