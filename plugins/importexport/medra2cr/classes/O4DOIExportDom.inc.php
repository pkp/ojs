...
class O4DOIExportDom extends DOIExportDom {
	...
	function _exportIssuesAsWork() {
		return $this->_exportIssuesAs == O4DOI_ISSUE_AS_WORK_2cr;
	}
	...
	function _setSchemaInfo($schema) {
		static $schemaInfos = array(
			...
			O4DOI_ISSUE_AS_MANIFESTATION_2cr => array(
	...
	function &_objectElement(&$object) {
		...
		switch ($this->_getSchema()) {
			case O4DOI_ISSUE_AS_WORK_2cr:
			case O4DOI_ISSUE_AS_MANIFESTATION_2cr:
	...
	function &_serialVersionElement($issn, $productForm) {
		...
		// ISSN
		if (!empty($issn)) {
			//Bug corretto qui https://github.com/pkp/ojs/commit/73cbb0c627e423ad18d930b7057c743be3dac9eb
	...
	function &_contributorElement(&$author, $objectLocalePrecedence) {
		...
		if (!empty($orcid)) {
			$nameIdentifierElement = XMLCustomWriter::createElement($this->getDoc(), 'NameIdentifier');
			XMLCustomWriter::createChildWithText($this->getDoc(), $nameIdentifierElement, 'NameIDType', '21');
			XMLCustomWriter::createChildWithText($this->getDoc(), $nameIdentifierElement, 'IDValue', $orcid);
			XMLCustomWriter::appendChild($contributorElement, $nameIdentifierElement);
		}
		...
		// NamesBeforeKey 
		$NamesBeforeKey = $author->getFirstName(true);
		assert(!empty($NamesBeforeKey));
		XMLCustomWriter::createChildWithText($this->getDoc(), $contributorElement, 'NamesBeforeKey', $NamesBeforeKey);

		// KeyNames 
		$KeyNames = $author->getLastName(true);
		assert(!empty($KeyNames));
		XMLCustomWriter::createChildWithText($this->getDoc(), $contributorElement, 'KeyNames', $KeyNames);
