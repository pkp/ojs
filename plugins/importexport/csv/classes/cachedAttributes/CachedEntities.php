...
class CachedEntities
{
    ...
    {
        return self::$userGroupIds[$journalPath] ??= Repo::userGroup()->getArrayIdByRoleId(Role::ROLE_ID_AUTHOR, $journalId)[0] ?? null;
    }
    ...
    {
        return self::$userGroups[$journalId] ??= UserGroup::query()
            ->select('user_groups.*')
            ->join('user_group_settings', 'user_groups.user_group_id', '=', 'user_group_settings.user_group_id')
            ->withContextIds([$journalId])
            ->get()
            ->toArray();
    }

    public static function getCachedUserGroupByName(string $name, int $journalId, string $locale): mixed
    {
        $uniquekey = "{$name}_{$locale}_{$journalId}";
        if (isset(self::$userGroups[$uniquekey])) {
            return self::$userGroups[$uniquekey];
        }
        ...
        foreach ($userGroups as $userGroup) {
            if (mb_strtolower($userGroup['name'][$locale]) === mb_strtolower($name)) {
                return self::$userGroups[$uniquekey] = $userGroup;
            }
        }
