...
/**
 * @file plugins/importexport/pubmed/filter/IssuePubMedXmlFilter.inc.php
 *
 * Copyright (c) 2014-2017 Simon Fraser University
 * Copyright (c) 2000-2017 John Willinsky
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class IssuePubMedXmlFilter
 * @ingroup plugins_importexport_pubmed
 *
 * @brief Base class that converts a set of issues to a PubMed XML document
 */

import('lib.pkp.classes.filter.PersistableFilter');

class IssuePubMedXmlFilter extends PersistableFilter {
	/**
	 * Constructor
	 * @param $filterGroup FilterGroup
	 */
	function __construct($filterGroup) {
		$this->setDisplayName('PubMed XML issue export');
	...
	function getClassName() {
		return 'plugins.importexport.pubmed.filter.IssuePubMedXmlFilter';
	}
	...
	function createIssueNode($doc, $issue) {
		...
		import('plugins.importexport.pubmed.filter.PubMedFilterHelper');
		$pubmedFilterHelper = new PubMedFilterHelper();
		$issueNode->appendChild($pubmedFilterHelper->createIssueIdentificationNode($this, $doc, $issue));
	...
	function addIssueGalleys($doc, $issueNode, $issue) {
		$filterDao = DAORegistry::getDAO('FilterDAO');
		$pubmedExportFilters = $filterDao->getObjectsByGroup('issuegalley=>pubmed-xml');
		assert(count($pubmedExportFilters)==1); // Assert only a single serialization filter
		$exportFilter = array_shift($pubmedExportFilters);
		$exportFilter->setDeployment($this->getDeployment());
