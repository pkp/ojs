...
class Collector extends \PKP\submission\Collector
{
    ...
    public function getQueryBuilder(): Builder
    {
        ...
        // add OJS-specific continuous publication and issueless publication filters
        $q->when(
            ...
            fn (Builder $query) => $query
                ...
                // Consider issueless publications
                ->where(function (Builder $query) {
                    $query
                        ->whereNull('publication_cp.issue_id')
                        ->where('publication_cp.published', true);
                })
                // Consider continuous publications e.g. attached to future issue which not published
                // but the publication is published
                ->orWhere(function (Builder $query) {
                    $query
                        ->whereNotNull('publication_cp.issue_id')
                        ->where('publication_cp.published', false);
                })
        );
