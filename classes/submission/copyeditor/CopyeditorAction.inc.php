...
class CopyeditorAction extends Action {
	...
	function completeCopyedit($copyeditorSubmission, $send, $request) {
		...
		if (!$email->isEnabled() || ($send && !$email->hasErrors())) {
			...
			SubmissionLog::logEvent($request, $copyeditorSubmission, SUBMISSION_LOG_COPYEDIT_INITIAL, 'log.copyedit.initialEditComplete', array('copyeditorName' => $user->getFullName()));
	...
	function completeFinalCopyedit($copyeditorSubmission, $send, $request) {
		...
		if (!$email->isEnabled() || ($send && !$email->hasErrors())) {
			...
			SubmissionLog::logEvent($request, $copyeditorSubmission, SUBMISSION_LOG_COPYEDIT_FINAL, 'log.copyedit.finalEditComplete', Array('copyeditorName' => $user->getFullName()));
	...
	function copyeditUnderway(&$copyeditorSubmission, $request) {
		if (!HookRegistry::call('CopyeditorAction::copyeditUnderway', array(&$copyeditorSubmission))) {
			...
			if (isset($update)) {
				...
				$user = $request->getUser();
				import('lib.pkp.classes.log.SubmissionLog');
				SubmissionLog::logEvent($request, $copyeditorSubmission, SUBMISSION_LOG_COPYEDIT_INITIATE, 'log.copyedit.initiate', array('copyeditorName' => $user->getFullName()));
			}
		}
	}
	...
	function uploadCopyeditVersion($copyeditorSubmission, $copyeditStage, $request) {
		...
		if (isset($fileId) && $fileId != 0) {
			...
			// Add log
			import('lib.pkp.classes.log.SubmissionLog');
			SubmissionLog::logEvent($request, $copyeditorSubmission, SUBMISSION_LOG_COPYEDITOR_FILE, 'log.copyedit.copyeditorFile', array('copyeditorName' => $user->getFullName(), 'fileId' => $fileId));
		}
	}
