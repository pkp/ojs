...
|---|---|---|---|
| `ROLE_ID_SITE_ADMIN` | 1 | — (no scenario key) | Site-wide administrator. Outside any journal — the group has a **null context id**. |
...
The site administrator role has no scenario key: the installer's `admin` is
the roster's only administrator, and the suite keeps it **enabled and
unmerged** — every other suite depends on it.
