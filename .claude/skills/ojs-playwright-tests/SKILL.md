...
- `PLAYWRIGHT_BASE_PORT` / `PLAYWRIGHT_WORKERS` — worker 0's port (OJS 8000 · OMP 8100 · OPS 8200) and how many per-worker PHP servers to start (unset = auto-detect: performance cores where the OS exposes them, else CPU cores − 2 — the measured knee matches the P-core count, 2026-08-21)
