...
12. **Mailpit reads must go through `pkpMail.find({to, contains})`** — `inboxFor`/`latestTo` are now search-scoped (Mailpit's `/api/v1/messages?query=` ignores the query; only `/api/v1/search` filters), but `find` scoped to a **unique throwaway recipient address** remains the only parallel-safe shape; an inbox-wide `latestTo` can still race two mails to the SAME recipient. `contains` is a subject/body **content marker**, not a Mailpit tag — this install has none (`GET /api/v1/tags` → `[]`, verified 2026-07-29), so it narrows the recipient scope but can never replace it.
...
- **Dashboard search commits on Enter only** (`Search.vue` `@keydown.enter.prevent` — the old keyup-debounce note is dead; 2026-08-02, U25). `fill()` alone never filters and neither does typing: press Enter to commit. A committed search FLIPS the dashboard to a dedicated cross-status "Search Results" view (`currentViewId=search`) that removes the in-page search box (the side-nav searchbox owns the query there — scope by accessible name `/Search submissions, ID/` to dodge the two-searchbox collision). To assert on a specific status view (e.g. Declined), assert on that view's own list response (`status[]=…`), not through search. Known latent debt: U26's `findRowByTag` helper never presses Enter — its rows come off the unfiltered page 1, valid today but flaky on a long-lived DB; touch up when U26's suite is next open.
