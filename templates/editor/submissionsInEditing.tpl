...
<table width="100%" class="listing">
	...
	<tr valign="top"{if $highlightClass || $fastTracked} class="{$highlightClass|escape} {if $fastTracked}fastTracked{/if}"{/if}>
		...
		<td><a href="{url op="submissionEditing" path=$submission->getArticleId()}" class="action">{$submission->getLocalizedTitle()|strip_tags|truncate:40:"..."}</a></td>
...
{/if}
        <tr>
                <td colspan="9">{translate key="common.queue.editor.seeMoreSubmissions"}</td>
        </tr>
</table>
</div>
