...
<h3>{translate key="editor.article.selectReviewer"} <a href="https://submit.escholarship.org/help/journals/editors_h.html" target="_blank"><img src="{$baseUrl}/eschol/images/help_A.png"></a></h3>
...
<p>
	{** BLH 20110822 Comment out link. No longer using Enroll Existing User functionality **}
	{**<a class="action" href="{url op="enrollSearch" path=$articleId}">{translate key="sectionEditor.review.enrollReviewer"}</a>&nbsp;|&nbsp; **} 
	<a class="action" href="{url op="createReviewer" path=$articleId}">{translate key="sectionEditor.review.createReviewer"}</a>
	{foreach from=$reviewerDatabaseLinks item="link"}
		{if !empty($link.title) && !empty($link.url)}&nbsp;|&nbsp;<a href="{$link.url|escape}" target="_new" class="action">{$link.title|escape}</a>{/if}
	{/foreach}
</p>
