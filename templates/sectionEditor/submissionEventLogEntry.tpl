...
<tr>
	<td>
		...
		<tr>
			<td>
				<a href="#" onclick="confirmAction('{$requestPageUrl}/clearSubmissionEventLog/{$submission->getArticleId()}/{$logEntry->getLogId()}', '{translate|escape:"javascript" key="submission.event.confirmDeleteLogEntry"}')" class="tableButton">{translate key="submission.event.deleteLogEntry"}</a>
			</td>
		</tr>
		</table>
	</td>
