...
<tr>
	<td>
		...
		<tr>
			<td>
				<a href="{$requestPageUrl}/clearSubmissionEmailLog/{$submission->getArticleId()}/{$logEntry->getLogId()}" onclick="return confirm('{translate|escape:"javascript" key="submission.email.confirmDeleteLogEntry"}')" class="tableButton">{translate key="submission.email.deleteLogEntry"}</a>
			</td>
		</tr>
		</table>
	</td>
