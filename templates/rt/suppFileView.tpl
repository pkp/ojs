...
<table class="data">
	<tr>
		...
		<td class="value">{$suppFile->getSuppFileTitle()|escape|default:"&mdash;"}</td>
	</tr>
	<tr>
...
{if $suppFile->getRemoteURL()}
	<tr>
		<td class="label">{translate key="submission.layout.galleyRemoteURL"}</td>
		<td class="value"><a href="{$suppFile->getRemoteURL()|escape}">{$suppFile->getRemoteURL()|escape}</a></td>
	</tr>
{else}
	<tr>
		<td class="label">{translate key="common.fileName"}</td>
		<td class="value"><a href="{url page="article" op="downloadSuppFile" path=$articleId|to_array:$suppFile->getBestSuppFileId($currentJournal)}">{$suppFile->getFileName()|escape}</a></td>
	</tr>
	<tr>
