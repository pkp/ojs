...
<table width="100%" class="data">
	...
	<tr valign="top">
		<td class="label">{translate key="article.suppFilesAbbrev"}</td>		<td width="30%" class="value">
			{foreach name="suppFiles" from=$suppFiles item=suppFile}				<a href="{$requestPageUrl}/downloadFile/{$suppFile->getFileId()}" class="file">{$suppFile->getFileName()}</a> {$suppFile->getDateModified()|date_format:$dateFormatShort}<br />
			{foreachelse}
				{translate key="common.none"}
			{/foreach}
		</td>
	...
	<tr valign="top">
		...
		<td width="80%" colspan="2" class="data">{assign var="submitter" value=$submission->getUser()}{$submitter->getFullName()}</td>
	</tr>
	<tr valign="top">
