...
{foreach from=$publishedArticles item=article}
	...
	<li>
		...
		<em><a href="{url journal=$journal->getPath() page="issue" op="view" path=$issue->getBestIssueId()}">{$issue->getIssueIdentification()|escape}</a> - {$section->getLocalizedTitle()|escape}</em><br />
