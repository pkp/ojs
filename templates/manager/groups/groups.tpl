...
<form action="{$pageUrl}/manager/setBoardEnabled" method="post">
	...
	<input type="radio" id="boardEnabledOn" {if $boardEnabled}checked="checked" {/if}name="boardEnabled" value="1"/>&nbsp;<label for="boardEnabledOn">{translate key="manager.groups.enableBoard" aboutEditorialTeamUrl=$aboutEditorialTeamUrl}</label><br/>
	<input type="radio" id="boardEnabledOff" {if !$boardEnabled}checked="checked" {/if}name="boardEnabled" value="0"/>&nbsp;<label for="boardEnabledOff">{translate key="manager.groups.disableBoard" aboutEditorialTeamUrl=$aboutEditorialTeamUrl peopleManagementUrl=$peopleManagementUrl}</label><br/>
