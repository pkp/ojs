...
installed = On
app_key = "base64:+4TCpvk+m/U5YHhbDdbbtbiNk3/sn5G62hqQXN0XNvM="

; The canonical URL to the OJS installation (excluding the trailing slash)
base_url = "http://127.0.0.1:8000"

; Enable strict mode. This wll more aggressively cause errors/warnings when
; deprecated behaviour exists in the codebase.
strict = Off

; Session cookie name
session_cookie_name = OJSTESTSID
...
; Locales installed on the site. The base seed's journal is multilingual
; (en + fr_CA), so fr_CA must be installed before bootstrap can seed it.
...
repository_id = "ojs-test.localhost"
...
; E2e test env: server-side outbound HTTP fails fast at this dead local port
; (nothing listens on :9) so tests never reach real external services. PKP
; wires [proxy] into Guzzle/Laravel HTTP; SMTP (Mailpit) is unaffected.
...
; Playwright test install: never run scheduled tasks at the end of a web request.
task_runner = Off
