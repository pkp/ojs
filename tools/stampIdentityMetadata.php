...
class StampJournalIdentityMetadata extends CommandLineTool
{
    ...
    /**
     * Journal abbreviation per locale, as used in citations and DOI deposits. Empty values are ignored.
     * e.g. ['en' => 'J. Old Name', 'de' => 'Z. Alter Titel']
     *
     * @var array<string,string>|null
     */
    ...
    {
        echo "Re-stamps journal identity metadata (name, abbreviation, ISSNs, publisher) onto published issues\n"
            . "and their published publications from the journal's current settings.\n"
            . "Unpublished content is skipped: it is stamped when it is published.\n"
            ...
            . "Use 'all' to stamp all published issues and all published publications, including those not assigned to an issue.\n";
    }

    /**
     * Re-stamp identity metadata on the targeted published issues and their published publications.
     */
    ...
    /**
     * Stamp all published issues and published publications in the context, including those not assigned to an issue.
     */
    ...
    /**
     * Stamp a single published issue and its published publications.
     */
    protected function stampIssue(int $issueId, Journal $context): void
    {
        ...
        if (!$issue->getData('published')) {
            printf("Skipping issue %d — not published.\n", $issueId);
            return;
        }
    ...
    protected function stampIssueFreePubsByYear(Journal $context, array $years): void
    {
        ...
        if ($count > 0) {
            printf("Stamped %d publication(s) without an issue published in the given year(s).\n", $count);
        } else {
            echo "No publications without an issue found published in the given year(s).\n";
        }
    }
