		    ...
		    foreach($articles as $article) {
				...
				if (empty($result)){
				   error_log("No articles in TOC!");
                    ...
                    $rawQualifiedArk = shell_exec("/apps/subi/subi/xtf-erep/control/tools/mintArk.py ojs $singleArticleID 2>&1"); 
                     error_log("ExecError:" . print_r($execError, 1));
                    ...
                    if ($crossRefXML !=""){
					  ...
					  $input = "_crossref: yes\n" . "_profile: crossref\n" . "_target: $escholURL\n" ."_coowners: $campusOwner[$journalPath]\n" . "crossref: $crossRefXML";
                        ...
                        if (!$isBackIssue){//if this is a new issue, then do the PUT request
                            error_log("New issue;give EZID a PUT request");
