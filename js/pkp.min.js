...
1E3)}})(jQuery);(function(a){a.pkp.controllers.UploaderHandler=function(c,b){this.parent(c,b);if(!c.is("div"))throw Error("An uploader widget controller can only be attached to a div!");var d;d={url:b.uploadUrl,flash_swf_url:b.baseUrl+"/lib/pkp/lib/vendor/moxiecode/plupload/js/Moxie.swf",silverlight_xap_url:b.baseUrl+"/lib/pkp/lib/vendor/moxiecode/plupload/js/Moxie.xap"};typeof b.filters&&(d.filters=b.filters);typeof b.resize&&(d.resize=b.resize);typeof b.browse_button&&(d.browse_button=b.browse_button);typeof b.multipart_params&&
...
this.callbackWrapper(this.handleError));this.pluploader.bind("FileUploaded",this.callbackWrapper(this.uploadComplete));this.pluploader.bind("QueueChanged",this.callbackWrapper(this.refreshUploader));this.getHtmlElement().find("#"+d.browse_button).click(function(c){c.preventDefault();a(this).siblings(".moxie-shim").find("input").click()});this.pluploader.refresh()};a.pkp.classes.Helper.inherits(a.pkp.controllers.UploaderHandler,a.pkp.classes.Handler);a.pkp.controllers.UploaderHandler.prototype.startUpload=
function(a,b){1<b.files.length&&b.removeFile(b.files[0]);this.updateStatus("uploading");b.start()};a.pkp.controllers.UploaderHandler.prototype.updateProgress=function(a,b,d){this.$progress.html(d.percent);this.$progressBar.css("width",d.percent+"%")};a.pkp.controllers.UploaderHandler.prototype.uploadComplete=function(c,b,d,e){c=a.parseJSON(e.response);b=d.name;c.status?("undefined"!==typeof c.uploadedFile&&(b=c.uploadedFile.name||c.uploadedFile.fileLabel,d.storedData=c.uploadedFile),this.$fileName.html(b),
this.updateStatus("complete"),this.$progress.html("0"),this.$progressBar.css("width",0)):this.showError(c.content)};a.pkp.controllers.UploaderHandler.prototype.handleError=function(a,b,d){this.showError(d.message)};a.pkp.controllers.UploaderHandler.prototype.showError=function(a){this.$progress.html("0");this.$progressBar.css("width",0);this.updateStatus("error");this.getHtmlElement().find(".pkpUploaderError").html(a)};a.pkp.controllers.UploaderHandler.prototype.refreshUploader=function(a,b){b.refresh()};
a.pkp.controllers.UploaderHandler.prototype.updateStatus=function(a){this.getHtmlElement().removeClass("loading waiting uploading error complete").addClass(a)};a.pkp.controllers.UploaderHandler.DEFAULT_PROPERTIES_={runtimes:"html5,flash,silverlight,html4",max_file_size:a.pkp.cons.UPLOAD_MAX_FILESIZE,multi_selection:!1,file_data_name:"uploadedFile",multipart:!0,headers:{browser_user_agent:navigator.userAgent},browse_button:"pkpUploaderButton",drop_element:"pkpUploaderDropZone"}})(jQuery);(function(a){a.pkp.controllers.AutocompleteHandler=function(c,b){var d;this.parent(c,b);this.sourceUrl_=b.sourceUrl;b.sourceUrl=void 0;this.jLabelText_=b.jLabelText;this.disableSync_=b.disableSync;d={};d.source=this.callbackWrapper(this.fetchAutocomplete);d.appendTo="#"+c.attr("id");d=a.extend({},this.self("DEFAULT_PROPERTIES_"),d,b);this.textInput=c.find(":text");this.disableSync_||this.textInput.keyup(this.callbackWrapper(this.synchronizeFields_));this.textInput.autocomplete(d);this.textInput.attr("title",
...
function(){var c=this.getHtmlElement().find("#reviewerSelectGridContainer").find("input:checked"),b=c.val();b&&(c=a.trim(c.parent().next().children("span").text()),a("#reviewerId").val(b),a('[id^="selectedReviewerName"]',this.getHtmlElement()).html(c),a("#searchGridAndButton").hide(),a("#regularReviewerForm").show())};a.pkp.controllers.grid.users.reviewer.AdvancedReviewerSearchHandler.prototype.handleRefresh_=function(a,b,d){d&&this.getHtmlElement().replaceWith(d)}})(jQuery);(function(a){a.pkp.controllers.grid.users.reviewer=a.pkp.controllers.grid.users.reviewer||{form:{}};a.pkp.controllers.grid.users.reviewer.form.EditReviewFormHandler=function(a,b){this.parent(a,b);a.change(this.callbackWrapper(this.handleFormChange));this.bind("urlInDivLoaded",this.handleFileListLoad_)};a.pkp.classes.Helper.inherits(a.pkp.controllers.grid.users.reviewer.form.EditReviewFormHandler,a.pkp.controllers.form.UserFormHandler);a.pkp.controllers.grid.users.reviewer.form.EditReviewFormHandler.prototype.handleFormChange=
...
e=this.handleJson(b),f=b.content,d=d.find('textarea[name="message"]'),g=tinyMCE.EditorManager.get(d.attr("id"));d.attr("data-variables",JSON.stringify(f.variables));g.setContent(f.body);return e.status};a.pkp.controllers.grid.users.stageParticipant.form.StageParticipantNotifyHandler.prototype.handleResponse=function(c,b){a.pkp.classes.Handler.getHandler(a("#queriesGrid .pkp_controllers_grid")).trigger("dataChanged");return this.parent("handleResponse",c,b)}})(jQuery);(function(a){a.pkp.controllers.grid.users.stageParticipant.form=a.pkp.controllers.grid.users.stageParticipant.form||{};a.pkp.controllers.grid.users.stageParticipant.form.AddParticipantFormHandler=function(c,b){this.parent(c,b);a("select[name^='filterUserGroupId']",c).change(this.callbackWrapper(this.addUserGroupId));this.addUserGroupId()};a.pkp.classes.Helper.inherits(a.pkp.controllers.grid.users.stageParticipant.form.AddParticipantFormHandler,a.pkp.controllers.form.ClientFormHandler);a.pkp.controllers.grid.users.stageParticipant.form.AddParticipantFormHandler.prototype.addUserGroupId=
function(){var c=this.getHtmlElement().find("select[name^='filterUserGroupId']").val();a("input[name='userGroupId']").val(c)}})(jQuery);(function(a){a.pkp.pages.reviewer=a.pkp.pages.reviewer||{};a.pkp.pages.reviewer.ReviewerTabHandler=function(a,b){this.parent(a,b);this.reviewStep_=b.reviewStep;this.bind("setStep",this.setStepHandler);this.getHtmlElement().tabs("option","disabled",this.getDisabledSteps(this.reviewStep_))};a.pkp.classes.Helper.inherits(a.pkp.pages.reviewer.ReviewerTabHandler,a.pkp.controllers.TabHandler);a.pkp.pages.reviewer.ReviewerTabHandler.prototype.reviewStep_=null;a.pkp.pages.reviewer.ReviewerTabHandler.prototype.setStepHandler=
