...
a);a&&"uninitialized"==a.toLowerCase()&&setTimeout(c,50)}catch(b){d("Server abort: ",b," (",b.name,")"),v(I),G&&clearTimeout(G),G=void 0}}var b=n.attr2("target"),e=n.attr2("action"),B=n.attr("enctype")||n.attr("encoding")||"multipart/form-data";u.setAttribute("target",y);(!q||/post/i.test(q))&&u.setAttribute("method","POST");e!=i.url&&u.setAttribute("action",i.url);!i.skipEncodingOverride&&(!q||/post/i.test(q))&&n.attr({encoding:"multipart/form-data",enctype:"multipart/form-data"});i.timeout&&(G=
setTimeout(function(){D=!0;v(O)},i.timeout));var F=[];try{if(i.extraData)for(var h in i.extraData)i.extraData.hasOwnProperty(h)&&(a.isPlainObject(i.extraData[h])&&i.extraData[h].hasOwnProperty("name")&&i.extraData[h].hasOwnProperty("value")?F.push(a('<input type="hidden" name="'+i.extraData[h].name+'">').val(i.extraData[h].value).appendTo(u)[0]):F.push(a('<input type="hidden" name="'+h+'">').val(i.extraData[h]).appendTo(u)[0]));i.iframeTarget||l.appendTo("body");m.attachEvent?m.attachEvent("onload",
v):m.addEventListener("load",v,!1);setTimeout(c,15);try{u.submit()}catch(z){document.createElement("form").submit.apply(u)}}finally{u.setAttribute("action",e),u.setAttribute("enctype",B),b?u.setAttribute("target",b):n.removeAttr("target"),a(F).remove()}}function v(c){if(!p.aborted&&!P)if(z=f(m),z||(d("cannot access response document"),c=I),c===O&&p)p.abort("timeout"),B.reject(p,"timeout");else if(c==I&&p)p.abort("server abort"),B.reject(p,"error","server abort");else if(z&&z.location.href!=i.iframeSrc||
D){m.detachEvent?m.detachEvent("onload",v):m.removeEventListener("load",v,!1);var c="success",b;try{if(D)throw"timeout";var e="xml"==i.dataType||z.XMLDocument||a.isXMLDoc(z);d("isXml="+e);if(!e&&(window.opera&&(null===z.body||!z.body.innerHTML))&&--T){d("requeing onLoad callback, DOM not available");setTimeout(v,250);return}var g=z.body?z.body:z.documentElement;p.responseText=g?g.innerHTML:null;p.responseXML=z.XMLDocument?z.XMLDocument:z;e&&(i.dataType="xml");p.getResponseHeader=function(a){return{"content-type":i.dataType}[a.toLowerCase()]};
g&&(p.status=Number(g.getAttribute("status"))||p.status,p.statusText=g.getAttribute("statusText")||p.statusText);var h=(i.dataType||"").toLowerCase(),j=/(json|script|text)/.test(h);if(j||i.textarea){var u=z.getElementsByTagName("textarea")[0];if(u)p.responseText=u.value,p.status=Number(u.getAttribute("status"))||p.status,p.statusText=u.getAttribute("statusText")||p.statusText;else if(j){var x=z.getElementsByTagName("pre")[0],n=z.getElementsByTagName("body")[0];x?p.responseText=x.textContent?x.textContent:
x.innerText:n&&(p.responseText=n.textContent?n.textContent:n.innerText)}}else"xml"==h&&(!p.responseXML&&p.responseText)&&(p.responseXML=s(p.responseText));try{var e=p,g=i,y=e.getResponseHeader("content-type")||"",q="xml"===h||!h&&0<=y.indexOf("xml"),w=q?e.responseXML:e.responseText;q&&"parsererror"===w.documentElement.nodeName&&a.error&&a.error("parsererror");g&&g.dataFilter&&(w=g.dataFilter(w,h));"string"===typeof w&&("json"===h||!h&&0<=y.indexOf("json")?w=A(w):("script"===h||!h&&0<=y.indexOf("javascript"))&&
a.globalEval(w));F=w}catch(r){c="parsererror",p.error=b=r||c}}catch(t){d("error caught: ",t),c="error",p.error=b=t||c}p.aborted&&(d("upload aborted"),c=null);p.status&&(c=200<=p.status&&300>p.status||304===p.status?"success":"error");"success"===c?(i.success&&i.success.call(i.context,F,"success",p),B.resolve(p.responseText,"success",p),k&&a.event.trigger("ajaxSuccess",[p,i])):c&&(void 0===b&&(b=p.statusText),i.error&&i.error.call(i.context,p,c,b),B.reject(p,"error",b),k&&a.event.trigger("ajaxError",
[p,i,b]));k&&a.event.trigger("ajaxComplete",[p,i]);k&&!--a.active&&a.event.trigger("ajaxStop");i.complete&&i.complete.call(i.context,p,c);P=!0;i.timeout&&clearTimeout(G);setTimeout(function(){i.iframeTarget?l.attr("src",i.iframeSrc):l.remove();p.responseXML=null},100)}}var u=n[0],x,i,k,y,l,m,p,D,G,B=a.Deferred();B.abort=function(a){p.abort(a)};if(e)for(x=0;x<r.length;x++)e=a(r[x]),h?e.prop("disabled",!1):e.removeAttr("disabled");i=a.extend(!0,{},a.ajaxSettings,c);i.context=i.context||i;y="jqFormIO"+
(new Date).getTime();i.iframeTarget?(l=a(i.iframeTarget),(x=l.attr2("name"))?y=x:l.attr2("name",y)):(l=a('<iframe name="'+y+'" src="'+i.iframeSrc+'" />'),l.css({position:"absolute",top:"-1000px",left:"-1000px"}));m=l[0];p={aborted:0,responseText:null,responseXML:null,status:0,statusText:"n/a",getAllResponseHeaders:function(){},getResponseHeader:function(){},setRequestHeader:function(){},abort:function(c){var b="timeout"===c?"timeout":"aborted";d("aborting upload... "+b);this.aborted=1;try{m.contentWindow.document.execCommand&&
m.contentWindow.document.execCommand("Stop")}catch(e){}l.attr("src",i.iframeSrc);p.error=b;i.error&&i.error.call(i.context,p,b,c);k&&a.event.trigger("ajaxError",[p,i,b]);i.complete&&i.complete.call(i.context,p,b)}};(k=i.global)&&0===a.active++&&a.event.trigger("ajaxStart");k&&a.event.trigger("ajaxSend",[p,i]);if(i.beforeSend&&!1===i.beforeSend.call(i.context,p,i))return i.global&&a.active--,B.reject(),B;if(p.aborted)return B.reject(),B;if(e=u.clk)if((x=e.name)&&!e.disabled)i.extraData=i.extraData||
{},i.extraData[x]=e.value,"image"==e.type&&(i.extraData[x+".x"]=u.clk_x,i.extraData[x+".y"]=u.clk_y);var O=1,I=2,e=a("meta[name=csrf-token]").attr("content");if((x=a("meta[name=csrf-param]").attr("content"))&&e)i.extraData=i.extraData||{},i.extraData[x]=e;i.forceSync?g():setTimeout(g,10);var F,z,T=50,P,s=a.parseXML||function(a,c){window.ActiveXObject?(c=new ActiveXObject("Microsoft.XMLDOM"),c.async="false",c.loadXML(a)):c=(new DOMParser).parseFromString(a,"text/xml");return c&&c.documentElement&&
"parsererror"!=c.documentElement.nodeName?c:null},A=a.parseJSON||function(a){return window.eval("("+a+")")};return B}if(!this.length)return d("ajaxSubmit: skipping submit process - no element selected"),this;var q,i,n=this;"function"==typeof c?c={success:c}:void 0===c&&(c={});q=c.type||this.attr2("method");i=c.url||this.attr2("action");(i=(i="string"===typeof i?a.trim(i):"")||window.location.href||"")&&(i=(i.match(/^([^#]+)/)||[])[1]);c=a.extend(!0,{url:i,success:a.ajaxSettings.success,type:q||a.ajaxSettings.type,
iframeSrc:/^https/i.test(window.location.href||"")?"javascript:false":"about:blank"},c);i={};this.trigger("form-pre-serialize",[this,c,i]);if(i.veto)return d("ajaxSubmit: submit vetoed via form-pre-serialize trigger"),this;if(c.beforeSerialize&&!1===c.beforeSerialize(this,c))return d("ajaxSubmit: submit aborted via beforeSerialize callback"),this;var l=c.traditional;void 0===l&&(l=a.ajaxSettings.traditional);var r=[],m,s=this.formToArray(c.semantic,r);c.data&&(c.extraData=c.data,m=a.param(c.data,
l));if(c.beforeSubmit&&!1===c.beforeSubmit(s,this,c))return d("ajaxSubmit: submit aborted via beforeSubmit callback"),this;this.trigger("form-submit-validate",[s,this,c,i]);if(i.veto)return d("ajaxSubmit: submit vetoed via form-submit-validate trigger"),this;i=a.param(s,l);m&&(i=i?i+"&"+m:m);"GET"==c.type.toUpperCase()?(c.url+=(0<=c.url.indexOf("?")?"&":"?")+i,c.data=null):c.data=i;var t=[];c.resetForm&&t.push(function(){n.resetForm()});c.clearForm&&t.push(function(){n.clearForm(c.includeHidden)});
if(!c.dataType&&c.target){var g=c.success||function(){};t.push(function(b){var d=c.replaceTarget?"replaceWith":"html";a(c.target)[d](b).each(g,arguments)})}else c.success&&t.push(c.success);c.success=function(a,b,d){for(var e=c.context||this,f=0,g=t.length;f<g;f++)t[f].apply(e,[a,b,d||n,n])};if(c.error){var C=c.error;c.error=function(a,b,d){C.apply(c.context||this,[a,b,d,n])}}if(c.complete){var v=c.complete;c.complete=function(a,b){v.apply(c.context||this,[a,b,n])}}m=0<a("input[type=file]:enabled",
this).filter(function(){return""!==a(this).val()}).length;i="multipart/form-data"==n.attr("enctype")||"multipart/form-data"==n.attr("encoding");l=e&&f;d("fileAPI :"+l);var u;if(!1!==c.iframe&&(c.iframe||(m||i)&&!l))c.closeKeepAlive?a.get(c.closeKeepAlive,function(){u=b(s)}):u=b(s);else if((m||i)&&l){var x=new FormData;for(i=0;i<s.length;i++)x.append(s[i].name,s[i].value);if(c.extraData){i=a.param(c.extraData,c.traditional).split("&");l=i.length;m=[];var y,D;for(y=0;y<l;y++)i[y]=i[y].replace(/\+/g,
" "),D=i[y].split("="),m.push([decodeURIComponent(D[0]),decodeURIComponent(D[1])]);for(i=0;i<m.length;i++)m[i]&&x.append(m[i][0],m[i][1])}c.data=null;m=a.extend(!0,{},a.ajaxSettings,c,{contentType:!1,processData:!1,cache:!1,type:q||"POST"});c.uploadProgress&&(m.xhr=function(){var b=a.ajaxSettings.xhr();b.upload&&b.upload.addEventListener("progress",function(a){var b=0,d=a.loaded||a.position,e=a.total;a.lengthComputable&&(b=Math.ceil(100*(d/e)));c.uploadProgress(a,d,e,b)},!1);return b});m.data=null;
var A=m.beforeSend;m.beforeSend=function(a,b){b.data=c.formData?c.formData:x;A&&A.call(this,a,b)};u=a.ajax(m)}else u=a.ajax(c);n.removeData("jqxhr").data("jqxhr",u);for(m=0;m<r.length;m++)r[m]=null;this.trigger("form-submit-notify",[this,c]);return this};a.fn.ajaxForm=function(e){e=e||{};e.delegation=e.delegation&&a.isFunction(a.fn.on);if(!e.delegation&&0===this.length){var f=this.selector,h=this.context;if(!a.isReady&&f)return d("DOM not ready, queuing ajaxForm"),a(function(){a(f,h).ajaxForm(e)}),
...
"inline",d=h(c,f),e.data("selectBox-options",d),f){case "inline":e.append(d);break;case "dropdown":q(c),a("BODY").append(d)}});break;case "value":if(void 0===b)return a(this).val();a(this).each(function(){var c,d=b;c=a(this);c.val(d);d=c.val();null===d&&(d=c.children().first().val(),c.val(d));var e=c.data("selectBox-control");if(e){var f=c.data("selectBox-settings"),e=e.data("selectBox-options");q(c);e.find(".selectBox-selected").removeClass("selectBox-selected");e.find("A").each(function(){if("object"===
...
function(a){t(b,a)}).bind("keypress.selectBox",function(a){g(b,a)}).bind("mousedown.selectBox",function(c){a(c.target).is("A.selectBox-inline")&&c.preventDefault();e.hasClass("selectBox-focus")||e.focus()}).insertAfter(b),!b[0].style.height){var f=b.attr("size")?parseInt(b.attr("size")):5,l=e.clone().removeAttr("id").css({position:"absolute",top:"-9999em"}).show().appendTo("body");l.find(".selectBox-options").html("<li><a>\u00a0</a></li>");var q=parseInt(l.find(".selectBox-options A:first").html("&nbsp;").outerHeight());
l.remove();e.height(q*f)}}else f=a('<span class="selectBox-label" />'),l=a('<span class="selectBox-arrow" />'),f.attr("class",j(b)).text(k(b)),d=h(b,"dropdown"),d.appendTo("BODY"),e.data("selectBox-options",d).addClass("selectBox-dropdown").append(f).append(l).bind("mousedown.selectBox",function(a){e.hasClass("selectBox-menuShowing")?n():(a.stopPropagation(),d.data("selectBox-down-at-x",a.screenX).data("selectBox-down-at-y",a.screenY),i(b))}).bind("keydown.selectBox",function(a){t(b,a)}).bind("keypress.selectBox",
function(a){g(b,a)}).bind("open.selectBox",function(a,c){c&&!0===c._selectBox||i(b)}).bind("close.selectBox",function(a,c){c&&!0===c._selectBox||n()}).insertAfter(b),l=e.width()-l.outerWidth()-parseInt(f.css("paddingLeft"))-parseInt(f.css("paddingLeft")),f.width(l);C(e);b.addClass("selectBox").data("selectBox-control",e).data("selectBox-settings",m).hide()}})}return a(this)}})}(jQuery);(function(){function a(a){return function(c,b,d,e){var b=C(b,e,4),f=!w(c)&&g.keys(c),h=(f||c).length,i=0<a?0:h-1;3>arguments.length&&(d=c[f?f[i]:i],i+=a);for(var j=b,k=d;0<=i&&i<h;i+=a)var l=f?f[i]:i,k=j(k,c[l],l,c);return k}}function c(a){return function(c,b,d){for(var b=v(b,d),d=A(c),e=0<a?0:d-1;0<=e&&e<d;e+=a)if(b(c[e],e,c))return e;return-1}}function b(a,c,b){return function(d,e,f){var h=0,i=A(d);if("number"==typeof f)0<a?h=0<=f?f:Math.max(f+i,h):i=0<=f?Math.min(f+1,i):f+i+1;else if(b&&f&&i)return f=
b(d,e),d[f]===e?f:-1;if(e!==e)return f=c(q.call(d,h,i),g.isNaN),0<=f?f+h:-1;for(f=0<a?h:i-1;0<=f&&f<i;f+=a)if(d[f]===e)return f;return-1}}function d(a,c){var b=Q.length,d=a.constructor,d=g.isFunction(d)&&d.prototype||j,e="constructor";for(g.has(a,e)&&!g.contains(c,e)&&c.push(e);b--;)e=Q[b],e in a&&(a[e]!==d[e]&&!g.contains(c,e))&&c.push(e)}var e=this,f=e._,h=Array.prototype,j=Object.prototype,k=h.push,q=h.slice,i=j.toString,n=j.hasOwnProperty,l=Array.isArray,r=Object.keys,m=Function.prototype.bind,
s=Object.create,t=function(){},g=function(a){if(a instanceof g)return a;if(!(this instanceof g))return new g(a);this._wrapped=a};"undefined"!==typeof exports?("undefined"!==typeof module&&module.exports&&(exports=module.exports=g),exports._=g):e._=g;g.VERSION="1.8.3";var C=function(a,c,b){if(void 0===c)return a;switch(null==b?3:b){case 1:return function(b){return a.call(c,b)};case 2:return function(b,d){return a.call(c,b,d)};case 3:return function(b,d,e){return a.call(c,b,d,e)};case 4:return function(b,
d,e,f){return a.call(c,b,d,e,f)}}return function(){return a.apply(c,arguments)}},v=function(a,c,b){return null==a?g.identity:g.isFunction(a)?C(a,c,b):g.isObject(a)?g.matcher(a):g.property(a)};g.iteratee=function(a,c){return v(a,c,Infinity)};var u=function(a,c){return function(b){var d=arguments.length;if(2>d||null==b)return b;for(var e=1;e<d;e++)for(var f=arguments[e],g=a(f),h=g.length,i=0;i<h;i++){var j=g[i];if(!c||void 0===b[j])b[j]=f[j]}return b}},x=function(a){if(!g.isObject(a))return{};if(s)return s(a);
t.prototype=a;a=new t;t.prototype=null;return a},y=function(a){return function(c){return null==c?void 0:c[a]}},D=Math.pow(2,53)-1,A=y("length"),w=function(a){a=A(a);return"number"==typeof a&&0<=a&&a<=D};g.each=g.forEach=function(a,c,b){var c=C(c,b),d;if(w(a)){b=0;for(d=a.length;b<d;b++)c(a[b],b,a)}else{var e=g.keys(a),b=0;for(d=e.length;b<d;b++)c(a[e[b]],e[b],a)}return a};g.map=g.collect=function(a,c,b){for(var c=v(c,b),b=!w(a)&&g.keys(a),d=(b||a).length,e=Array(d),f=0;f<d;f++){var h=b?b[f]:f;e[f]=
...
d){if(b>d||void 0===b)return 1;if(b<d||void 0===d)return-1}return a.index-c.index}),"value")};var H=function(a){return function(c,b,d){var e={},b=v(b,d);g.each(c,function(d,f){var g=b(d,f,c);a(e,d,g)});return e}};g.groupBy=H(function(a,c,b){g.has(a,b)?a[b].push(c):a[b]=[c]});g.indexBy=H(function(a,c,b){a[b]=c});g.countBy=H(function(a,c,b){g.has(a,b)?a[b]++:a[b]=1});g.toArray=function(a){return!a?[]:g.isArray(a)?q.call(a):w(a)?g.map(a,g.identity):g.values(a)};g.size=function(a){return null==a?0:w(a)?
a.length:g.keys(a).length};g.partition=function(a,c,b){var c=v(c,b),d=[],e=[];g.each(a,function(a,b,f){(c(a,b,f)?d:e).push(a)});return[d,e]};g.first=g.head=g.take=function(a,c,b){return null==a?void 0:null==c||b?a[0]:g.initial(a,a.length-c)};g.initial=function(a,c,b){return q.call(a,0,Math.max(0,a.length-(null==c||b?1:c)))};g.last=function(a,c,b){return null==a?void 0:null==c||b?a[a.length-1]:g.rest(a,Math.max(0,a.length-c))};g.rest=g.tail=g.drop=function(a,c,b){return q.call(a,null==c||b?1:c)};g.compact=
function(a){return g.filter(a,g.identity)};var E=function(a,c,b,d){for(var e=[],f=0,d=d||0,h=A(a);d<h;d++){var i=a[d];if(w(i)&&(g.isArray(i)||g.isArguments(i))){c||(i=E(i,c,b));var j=0,k=i.length;for(e.length+=k;j<k;)e[f++]=i[j++]}else b||(e[f++]=i)}return e};g.flatten=function(a,c){return E(a,c,!1)};g.without=function(a){return g.difference(a,q.call(arguments,1))};g.uniq=g.unique=function(a,c,b,d){g.isBoolean(c)||(d=b,b=c,c=!1);null!=b&&(b=v(b,d));for(var d=[],e=[],f=0,h=A(a);f<h;f++){var i=a[f],
j=b?b(i,f,a):i;c?((!f||e!==j)&&d.push(i),e=j):b?g.contains(e,j)||(e.push(j),d.push(i)):g.contains(d,i)||d.push(i)}return d};g.union=function(){return g.uniq(E(arguments,!0,!0))};g.intersection=function(a){for(var c=[],b=arguments.length,d=0,e=A(a);d<e;d++){var f=a[d];if(!g.contains(c,f)){for(var h=1;h<b&&g.contains(arguments[h],f);h++);h===b&&c.push(f)}}return c};g.difference=function(a){var c=E(arguments,!0,!0,1);return g.filter(a,function(a){return!g.contains(c,a)})};g.zip=function(){return g.unzip(arguments)};
...
b||1,c=Math.max(Math.ceil((c-a)/b),0),d=Array(c),e=0;e<c;e++,a+=b)d[e]=a;return d};var M=function(a,c,b,d,e){if(!(d instanceof c))return a.apply(b,e);c=x(a.prototype);a=a.apply(c,e);return g.isObject(a)?a:c};g.bind=function(a,c){if(m&&a.bind===m)return m.apply(a,q.call(arguments,1));if(!g.isFunction(a))throw new TypeError("Bind must be called on a function");var b=q.call(arguments,2),d=function(){return M(a,d,c,this,b.concat(q.call(arguments)))};return d};g.partial=function(a){var c=q.call(arguments,
...
{};return b};g.delay=function(a,c){var b=q.call(arguments,2);return setTimeout(function(){return a.apply(null,b)},c)};g.defer=g.partial(g.delay,g,1);g.throttle=function(a,c,b){var d,e,f,h=null,i=0;b||(b={});var j=function(){i=!1===b.leading?0:g.now();h=null;f=a.apply(d,e);h||(d=e=null)};return function(){var k=g.now();!i&&!1===b.leading&&(i=k);var l=c-(k-i);d=this;e=arguments;0>=l||l>c?(h&&(clearTimeout(h),h=null),i=k,f=a.apply(d,e),h||(d=e=null)):!h&&!1!==b.trailing&&(h=setTimeout(j,l));return f}};
...
b[d];return c};g.functions=g.methods=function(a){var c=[],b;for(b in a)g.isFunction(a[b])&&c.push(b);return c.sort()};g.extend=u(g.allKeys);g.extendOwn=g.assign=u(g.keys);g.findKey=function(a,c,b){for(var c=v(c,b),b=g.keys(a),d,e=0,f=b.length;e<f;e++)if(d=b[e],c(a[d],d,a))return d};g.pick=function(a,c,b){var d={},e=a,f,h;if(null==e)return d;g.isFunction(c)?(h=g.allKeys(e),f=C(c,b)):(h=E(arguments,!1,!1,1),f=function(a,c,b){return c in b},e=Object(e));for(var i=0,j=h.length;i<j;i++){var k=h[i],l=e[k];
f(l,k,e)&&(d[k]=l)}return d};g.omit=function(a,c,b){if(g.isFunction(c))c=g.negate(c);else var d=g.map(E(arguments,!1,!1,1),String),c=function(a,c){return!g.contains(d,c)};return g.pick(a,c,b)};g.defaults=u(g.allKeys,!0);g.create=function(a,c){var b=x(a);c&&g.extendOwn(b,c);return b};g.clone=function(a){return!g.isObject(a)?a:g.isArray(a)?a.slice():g.extend({},a)};g.tap=function(a,c){c(a);return a};g.isMatch=function(a,c){var b=g.keys(c),d=b.length;if(null==a)return!d;for(var e=Object(a),f=0;f<d;f++){var h=
b[f];if(c[h]!==e[h]||!(h in e))return!1}return!0};var K=function(a,c,b,d){if(a===c)return 0!==a||1/a===1/c;if(null==a||null==c)return a===c;a instanceof g&&(a=a._wrapped);c instanceof g&&(c=c._wrapped);var e=i.call(a);if(e!==i.call(c))return!1;switch(e){case "[object RegExp]":case "[object String]":return""+a===""+c;case "[object Number]":return+a!==+a?+c!==+c:0===+a?1/+a===1/c:+a===+c;case "[object Date]":case "[object Boolean]":return+a===+c}e="[object Array]"===e;if(!e){if("object"!=typeof a||
...
g.isEqual=function(a,c){return K(a,c)};g.isEmpty=function(a){return null==a?!0:w(a)&&(g.isArray(a)||g.isString(a)||g.isArguments(a))?0===a.length:0===g.keys(a).length};g.isElement=function(a){return!!(a&&1===a.nodeType)};g.isArray=l||function(a){return"[object Array]"===i.call(a)};g.isObject=function(a){var c=typeof a;return"function"===c||"object"===c&&!!a};g.each("Arguments Function String Number Date RegExp Error".split(" "),function(a){g["is"+a]=function(c){return i.call(c)==="[object "+a+"]"}});
g.isArguments(arguments)||(g.isArguments=function(a){return g.has(a,"callee")});"function"!=typeof/./&&"object"!=typeof Int8Array&&(g.isFunction=function(a){return"function"==typeof a||!1});g.isFinite=function(a){return isFinite(a)&&!isNaN(parseFloat(a))};g.isNaN=function(a){return g.isNumber(a)&&a!==+a};g.isBoolean=function(a){return!0===a||!1===a||"[object Boolean]"===i.call(a)};g.isNull=function(a){return null===a};g.isUndefined=function(a){return void 0===a};g.has=function(a,c){return null!=a&&
n.call(a,c)};g.noConflict=function(){e._=f;return this};g.identity=function(a){return a};g.constant=function(a){return function(){return a}};g.noop=function(){};g.property=y;g.propertyOf=function(a){return null==a?function(){}:function(c){return a[c]}};g.matcher=g.matches=function(a){a=g.extendOwn({},a);return function(c){return g.isMatch(c,a)}};g.times=function(a,c,b){for(var d=Array(Math.max(0,a)),c=C(c,b,1),b=0;b<a;b++)d[b]=c(b);return d};g.random=function(a,c){null==c&&(c=a,a=0);return a+Math.floor(Math.random()*
(c-a+1))};g.now=Date.now||function(){return(new Date).getTime()};l={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#x27;","`":"&#x60;"};u=g.invert(l);y=function(a){var c=function(c){return a[c]},b="(?:"+g.keys(a).join("|")+")",d=RegExp(b),e=RegExp(b,"g");return function(a){a=null==a?"":""+a;return d.test(a)?a.replace(e,c):a}};g.escape=y(l);g.unescape=y(u);g.result=function(a,c,b){c=null==a?void 0:a[c];void 0===c&&(c=b);return g.isFunction(c)?c.call(a):c};var U=0;g.uniqueId=function(a){var c=
++U+"";return a?a+c:c};g.templateSettings={evaluate:/<%([\s\S]+?)%>/g,interpolate:/<%=([\s\S]+?)%>/g,escape:/<%-([\s\S]+?)%>/g};var J=/(.)^/,S={"'":"'","\\":"\\","\r":"r","\n":"n","\u2028":"u2028","\u2029":"u2029"},R=/\\|'|\r|\n|\u2028|\u2029/g,p=function(a){return"\\"+S[a]};g.template=function(a,c,b){!c&&b&&(c=b);var c=g.defaults({},c,g.templateSettings),b=RegExp([(c.escape||J).source,(c.interpolate||J).source,(c.evaluate||J).source].join("|")+"|$","g"),d=0,e="__p+='";a.replace(b,function(c,b,f,
g,h){e+=a.slice(d,h).replace(R,p);d=h+c.length;b?e+="'+\n((__t=("+b+"))==null?'':_.escape(__t))+\n'":f?e+="'+\n((__t=("+f+"))==null?'':__t)+\n'":g&&(e+="';\n"+g+"\n__p+='");return c});e+="';\n";c.variable||(e="with(obj||{}){\n"+e+"}\n");e="var __t,__p='',__j=Array.prototype.join,print=function(){__p+=__j.call(arguments,'');};\n"+e+"return __p;\n";try{var f=new Function(c.variable||"obj","_",e)}catch(h){throw h.source=e,h;}b=function(a){return f.call(this,a,g)};b.source="function("+(c.variable||"obj")+
"){\n"+e+"}";return b};g.chain=function(a){a=g(a);a._chain=!0;return a};var L=function(a,c){return a._chain?g(c).chain():c};g.mixin=function(a){g.each(g.functions(a),function(c){var b=g[c]=a[c];g.prototype[c]=function(){var a=[this._wrapped];k.apply(a,arguments);return L(this,b.apply(g,a))}})};g.mixin(g);g.each("pop push reverse shift sort splice unshift".split(" "),function(a){var c=h[a];g.prototype[a]=function(){var b=this._wrapped;c.apply(b,arguments);("shift"===a||"splice"===a)&&0===b.length&&
...
this.$linkActionElement.focus();this.$modal_.remove();return this.parent("finish")}})(jQuery);(function(a){a.pkp.classes.notification=a.pkp.classes.notification||{};a.pkp.classes.notification.NotificationHelper=function(){};a.pkp.classes.notification.NotificationHelper.redirectNotifyUserEvent=function(c,b){var d,e,f,h,j,k,q,i,n,l,r;if(void 0!==b.content)c.getHtmlElement().parent().trigger("notifyUser",[b]);else{d=c.getHtmlElement();e=!1;!(c instanceof a.pkp.controllers.SiteHandler)&&0<a(b).parents(".pkp_controllers_grid").length&&(d.parent().trigger("notifyUser"),e=!0);d=a(".pkp_notification",
d);f=[];h=0;for(j=d.length;h<j;h++)if(q=a(d[h]),!(0<q.parents(":hidden").length)){k=q.parents();n=0;for(l=k.length;n<l;n++)if(i=a(k[n]).data("pkp.handler"),i instanceof a.pkp.classes.Handler){r=a(k[n]);break}if(r.has(b[0]).length||r[0]===b[0]){if(0<q.parents(".ui-accordion:first").length&&(k=q.parents(".ui-accordion:first"),!k.has(b[0])))continue;f.push(q)}}if(f.length)for(h=f.length-1;-1<h;h--)f[h].triggerHandler("notifyUser");else e||c.getHtmlElement().parent().trigger("notifyUser")}}})(jQuery);(function(a){a.pkp.classes.features=a.pkp.classes.features||{};a.pkp.classes.features.Feature=function(a,b){this.gridHandler=a;this.options_=b;this.addFeatureHtml(this.getGridHtmlElement(),b)};a.pkp.classes.features.Feature.prototype.gridHandler=null;a.pkp.classes.features.Feature.prototype.options_=null;a.pkp.classes.features.Feature.prototype.setOptions=function(a){this.options_=a};a.pkp.classes.features.Feature.prototype.getOptions=function(){return this.options_};a.pkp.classes.features.Feature.prototype.init=
...
void 0!=c.newTopRow&&(b=this.gridHandler.getRows(),d.currentItemsPerPage==b.length&&this.gridHandler.deleteElement(b.last(),!0),b=c.newTopRow,this.gridHandler.insertOrReplaceElement(b,!0));return!1};a.pkp.classes.features.PagingFeature.prototype.configPagingLinks_=function(){var c,b,d,e,f,h,j,k;c=this.getOptions();if(b=a("div.gridPaging",this.getGridHtmlElement())){k=this.callbackWrapper(function(b,d){h=RegExp("[?&]"+c.pageParamName+"(?:=([^&]*))?","i");j=h.exec(a(d.target).attr("href"));null!=j&&
(c.currentPage=parseInt(j[1],10),this.getGridHtmlElement().trigger("dataChanged"));return!1},this);b=b.find("a").not(".showMoreItems").not(".showLessItems");d=0;for(e=b.length;d<e;d++)f=a(b[d]),f.click(k)}};a.pkp.classes.features.PagingFeature.prototype.configItemsPerPageElement_=function(){var c,b,d,e,f,h;c=this.getOptions();if(b=a("div.gridPaging",this.getGridHtmlElement()))if(h=this.callbackWrapper(function(b,d){c.currentItemsPerPage=parseInt(a("option",d.target).filter(":selected").attr("value"),
...
dataType:"json",async:!1})};a.pkp.controllers.SiteHandler.prototype.updateHeaderHandler_=function(){a.pkp.classes.Handler.getHandler(a("#navigationUserWrapper")).reload()};a.pkp.controllers.SiteHandler.prototype.callWhenClickOutsideHandler_=function(a,b,d){void 0!==d.container&&(a=d.container.attr("id"),d.clear?delete this.outsideClickChecks_[a]:void 0!==d.callback&&(this.outsideClickChecks_[a]=d))};a.pkp.controllers.SiteHandler.prototype.mouseDownHandler_=function(c,b){var d;if(!a.isEmptyObject(this.outsideClickChecks_))for(d in this.outsideClickChecks_)this.processOutsideClickCheck_(this.outsideClickChecks_[d],
...
b,d){this.getHtmlElement();var e=this;a.get(d.tabsUrl,function(a){e.replaceWith(a.content)})};a.pkp.controllers.TabHandler.prototype.addTab=function(c,b,d){var e=this.getHtmlElement(),c=e.children("ul").children("li").length+1,b=a("<a/>").text(d.title).attr("href",d.url),d=a("<a/>").addClass("close").text(a.pkp.locale.common_close).attr("href","#"),b=a("<li/>").append(b).append(d);d.click(function(){var c=a(this).closest("li"),b=a("#"+c.attr("aria-controls")),d,k;k=!1;b.find("form").each(function(){if(a.pkp.classes.Handler.getHandler(a(this)).formChangesTracked&&
!confirm(a.pkp.locale.form_dataHasChanged))return k=!0,!1});k||(b.find("form").each(function(){var c=a.pkp.classes.Handler.getHandler(a(this));c&&c.unregisterForm()}),d=c.eq(0).index(),e.tabs("option","active")==d&&e.tabs("option","active",d-1),c.remove(),b.remove(),e.tabs("refresh"))});e.children("ul").append(b);e.tabs("refresh");e.tabs("option","active",c-1)};a.pkp.controllers.TabHandler.prototype.getCurrentTab=function(){return this.$currentTab_};a.pkp.controllers.TabHandler.prototype.getCurrentTabIndex=
...
null),"json")):this.enableControls(),this.callFeaturesHook("addElement",d));return!1};a.pkp.controllers.listbuilder.ListbuilderHandler.prototype.fetchOptionsResponseHandler_=function(c,b){var d=this.handleJson(b),e=this.getHtmlElement(),f=[],h,j,k,q,i,n,l,r,m,s;if(!1!==d){e.find(".gridCellDisplay :input").each(function(c,b){f[c]=a(b).val()});e=e.find(".gridRowEdit:visible .selectMenu:input");h=0;for(j=e.length;h<j;h++){k=a(e[h]);q=k.parents(".gridCellContainer");i=0;k.children().empty();n=null;for(n in d.content[h])if(n!=
a.pkp.cons.LISTBUILDER_OPTGROUP_LABEL)if("object"==typeof d.content[h][n]){if(void 0!==d.content[h][a.pkp.cons.LISTBUILDER_OPTGROUP_LABEL]&&"object"==typeof d.content[h][a.pkp.cons.LISTBUILDER_OPTGROUP_LABEL]&&(r=d.content[h][a.pkp.cons.LISTBUILDER_OPTGROUP_LABEL][n])){m=a("<optgroup></optgroup>");m.attr("label",r);k.append(m);r=null;s=0;for(r in d.content[h][n])if(l=this.populatePulldown_(m,f,d.content[h][n][r],r))i++,s++;0===s&&m.remove()}}else(l=this.populatePulldown_(k,f,d.content[h][n],n))&&
i++;k=l;1===i&&k&&(k.attr("selected","selected"),this.availableOptions_=!1);0===i&&(q.find(".gridCellDisplay").show(),q.find(".gridCellEdit").hide())}}this.enableControls();return!1};a.pkp.controllers.listbuilder.ListbuilderHandler.prototype.populatePulldown_=function(c,b,d,e){var f=c.parents(".gridCellContainer").find(".gridCellDisplay :input").val(),h=!1,j;if(e!=f)for(j=0;j<b.length;j++)b[j]==e&&(h=!0);if(h)return!1;b=a("<option/>");b.attr("value",e);b.text(d);e==f&&b.attr("selected","selected");
...
300);c.trigger("pkpModalOpen",[c])};a.pkp.controllers.modal.ModalHandler.prototype.modalClose=function(){var a=this,b=this.getHtmlElement(),d=b.find("form").first(),e;if(1==d.length&&(e={closePermitted:!0},d.trigger("containerClose",[e]),!e.closePermitted))return!1;b.removeClass("is_visible");this.trigger("pkpModalClose");setTimeout(function(){var d=a.options.closeCleanVueInstances,e,j;if(d.length)for(j=0;j<d.length;j++)e=d[j],"undefined"!==typeof pkp.registry._instances[e]&&(e=pkp.registry._instances[e],
...
e=d.find("#notificationsPopover"),f=d.find("#notificationsToggle");e.toggle();f.toggleClass("expandedIndicator");f.hasClass("expandedIndicator")?(this.trigger("callWhenClickOutside",[{container:d,callback:this.callbackWrapper(this.appendToggleIndicator_)}]),setTimeout(this.callbackWrapper(this.setPopoverSize_),500)):(e.css("height","").css("overflow-y",""),b&&this.trigger("callWhenClickOutside",[{container:d,clear:!0}]))};a.pkp.pages.header.TasksHandler.prototype.setPopoverSize_=function(){var c=
this.getHtmlElement().find("#notificationsPopover"),b=a(window).height()-60;b<c.height()&&c.height(b).css("overflow-y","scroll")};a.pkp.pages.header.TasksHandler.prototype.fetchUnreadNotificationsCountHandler_=function(){a.get(this.options_.fetchUnreadNotificationsCountUrl,this.callbackWrapper(this.updateUnreadNotificationsCountHandler_),"json")};a.pkp.pages.header.TasksHandler.prototype.updateUnreadNotificationsCountHandler_=function(a,b){var d=this.getHtmlElement().find("#unreadNotificationCount");
d.html(b.content);"0"==b.content?d.removeClass("hasTasks"):d.addClass("hasTasks")}})(jQuery);(function(a){a.pkp.pages.submission=a.pkp.pages.submission||{};a.pkp.pages.submission.SubmissionTabHandler=function(a,b){this.parent(a,b);this.submissionProgress_=b.submissionProgress;this.cancelUrl_=b.cancelUrl;this.cancelConfirmText_=b.cancelConfirmText;this.bind("setStep",this.setStepHandler);this.bind("formCanceled",this.formCanceledHandler);this.getHtmlElement().tabs("option","disabled",this.getDisabledSteps(this.submissionProgress_))};a.pkp.classes.Helper.inherits(a.pkp.pages.submission.SubmissionTabHandler,
