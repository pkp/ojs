...
a.event.trigger("ajaxStop");f.complete&&f.complete(l,b?"success":"error");setTimeout(function(){i.remove();l.responseXML=null},100)}}var e=j[0];if(a(":input[name=submit]",e).length)alert('Error: Form elements must not be named "submit".');else{var f=a.extend({},a.ajaxSettings,b),g=a.extend(!0,{},a.extend(!0,{},a.ajaxSettings),f),h="jqFormIO"+(new Date).getTime(),i=a('<iframe id="'+h+'" name="'+h+'" src="'+f.iframeSrc+'" />'),k=i[0];i.css({position:"absolute",top:"-1000px",left:"-1000px"});var l={aborted:0,
...
g.type&&(b.extraData[m+".x"]=e.clk_x,b.extraData[m+".y"]=e.clk_y))}setTimeout(function(){var c=j.attr("target"),g=j.attr("action");e.setAttribute("target",h);"POST"!=e.getAttribute("method")&&e.setAttribute("method","POST");e.getAttribute("action")!=f.url&&e.setAttribute("action",f.url);b.skipEncodingOverride||j.attr({encoding:"multipart/form-data",enctype:"multipart/form-data"});f.timeout&&setTimeout(function(){t=!0;d()},f.timeout);var l=[];try{if(b.extraData)for(var m in b.extraData)l.push(a('<input type="hidden" name="'+
m+'" value="'+b.extraData[m]+'" />').appendTo(e)[0]);i.appendTo("body");k.attachEvent?k.attachEvent("onload",d):k.addEventListener("load",d,!1);e.submit()}finally{e.setAttribute("action",g),c?e.setAttribute("target",c):j.removeAttr("target"),a(l).remove()}},10);var u=50}}}if(!this.length)return c("ajaxSubmit: skipping submit process - no element selected"),this;"function"==typeof b&&(b={success:b});var e=a.trim(this.attr("action"));e&&(e=(e.match(/^([^#]+)/)||[])[1]);e=e||window.location.href||"";
...
Array)for(var h in b.data[g])f.push({name:g,value:b.data[g][h]});else f.push({name:g,value:b.data[g]})}if(b.beforeSubmit&&!1===b.beforeSubmit(f,this,b))return c("ajaxSubmit: submit aborted via beforeSubmit callback"),this;this.trigger("form-submit-validate",[f,this,b,e]);if(e.veto)return c("ajaxSubmit: submit vetoed via form-submit-validate trigger"),this;g=a.param(f);"GET"==b.type.toUpperCase()?(b.url+=(0<=b.url.indexOf("?")?"&":"?")+g,b.data=null):b.data=g;var j=this,i=[];b.resetForm&&i.push(function(){j.resetForm()});
b.clearForm&&i.push(function(){j.clearForm()});if(!b.dataType&&b.target){var k=b.success||function(){};i.push(function(c){a(b.target).html(c).each(k,arguments)})}else b.success&&i.push(b.success);b.success=function(a,c){for(var d=0,e=i.length;d<e;d++)i[d].apply(b,[a,c,j])};g=a("input:file",this).fieldValue();h=!1;for(e=0;e<g.length;e++)g[e]&&(h=!0);g.length&&!1!==b.iframe||b.iframe||h?b.closeKeepAlive?a.get(b.closeKeepAlive,d):d():a.ajax(b);this.trigger("form-submit-notify",[this,b]);return this};
...
typeof c&&(c=!0);if(c&&(!e||a.disabled||"reset"==f||"button"==f||("checkbox"==f||"radio"==f)&&!a.checked||("submit"==f||"image"==f)&&a.form&&a.form.clk!=a||"select"==g&&-1==a.selectedIndex))return null;if("select"==g){var h=a.selectedIndex;if(0>h)return null;for(var e=[],g=a.options,j=(f="select-one"==f)?h+1:g.length,h=f?h:0;h<j;h++){var i=g[h];if(i.selected){var k=i.value;k||(k=i.attributes&&i.attributes.value&&!i.attributes.value.specified?i.text:i.value);if(f)return k;e.push(k)}}return e}return a.value};
